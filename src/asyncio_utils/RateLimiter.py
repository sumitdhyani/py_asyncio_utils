...
class RateLimiter:
    def __init__(self, rate: int, per: timedelta):
        self.ringBuffer = RingBuffer(rate)
        self.rate = rate
        self.per = per
        self.pendingTasks : list[Callback] = []

    def bandWidthAvailable(self) -> bool:
        return not self.ringBuffer.is_full() or self.ringBuffer.get_front() + self.per < datetime.now()
    ...
    async def push(self, task: Callback) -> None:
      ...
      if self.bandWidthAvailable():
        ...
        task : asyncio.Task = asyncio.create_task(asyncio.sleep((self.ringBuffer.get_front() + self.per).total_seconds()))
        task.add_done_callback(lambda coro_object: asyncio.create_task(self.onBandWidthAvailable()))
